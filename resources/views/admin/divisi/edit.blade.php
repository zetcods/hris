    ...
    <div style="background: #1e293b; padding: 30px; border-radius: 12px; box-shadow: 0 5px 20px rgba(0,0,0,0.35); border: 1px solid #334155;">
        <form action="{{ route('divisi.update', $divisi->id) }}" method="POST">
            ...
            <input 
                ...
                required 
                placeholder="Misal: Human Resource Department"
            ...
            >
                <option value="karyawan" {{ old('role', $divisi->role) == 'karyawan' ? 'selected' : '' }}>Karyawan (Akses Dashboard Karyawan)</option>
                <option value="admin" {{ old('role', $divisi->role) == 'admin' ? 'selected' : '' }}>Admin (Akses Dashboard Admin)</option>
            </select>
            @error('role')
              <p style="color:#ef4444; margin-top:4px; font-size: 0.9rem;">{{ $message }}</p>
            @enderror
