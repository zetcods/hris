    ...
    <table style="width: 100%; border-collapse: collapse;">
      <thead>
        <tr style="background: #111827; color: #38bdf8; text-align: left; font-size: 0.95rem;">
          <th style="padding: 14px 16px;">Nama Divisi</th>
      ...
      <tbody>
        @foreach ($divisi as $d)
          ...
          >
            <td style="padding: 12px 16px; font-weight: 500;">{{ $d->nama_divisi }}</td>
            <td>{{ $d->karyawan_count }} Karyawan</td>
            <td style="text-align: center;">
              <a href="{{ route('divisi.show', $d->id) }}" 
                 style="background: #10b981; color: #fff; padding: 6px 12px; border-radius: 6px;
                        text-decoration: none; font-weight: 600; margin-right: 5px; font-size: 0.9rem;
                        box-shadow: 0 2px 5px rgba(16,185,129,0.4); transition: .3s;">
                 👀 Lihat
              </a>
              ...
              <form id="delete-form-{{ $d->id }}" 
                    action="{{ route('divisi.destroy', $d->id) }}" 
        ...
        @if ($divisi->isEmpty())
          <tr>
            <td colspan="3" style="text-align:center; padding: 25px; color: #94a3b8;">
...
document.querySelectorAll('.btn-delete').forEach(btn => {
  btn.addEventListener('click', function() {
    ...
    Swal.fire({
      ...
      text: "Data divisi dan semua karyawannya akan dihapus!",
      icon: 'warning',
